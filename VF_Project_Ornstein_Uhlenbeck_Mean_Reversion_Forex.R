...
# Computes the rolling Z-score of a price series over a specified lookback window
...
# Implements a mean-reversion trading strategy using Z-score signals, managing entries, exits, stop-loss, and take-profit rules while tracking positions and cumulative P&L.
...
# Calculates key performance metrics (return, Sharpe ratio, drawdown, win rate, etc.) based on the strategy's P&L to evaluate its effectiveness.
...
# Generates three plots (price with signals, Z-score, and cumulative P&L) to visually analyze the trading strategy's behavior and performance for a given symbol.
...
# Runs the Ornstein Uhlenbeck trading strategy across multiple symbols, evaluates performance for each, and compiles a summary table of key metrics.
...
# Compares the Ornstein Uhlenbeck strategy to a Buy & Hold approach for each symbol by calculating and summarizing returns and Sharpe ratios.
...
extract_trades <- function(prices, strategy_results, symbol) {
  n <- length(prices)
  position <- strategy_results$position
  entry_price <- strategy_results$entry_price
  pnl <- strategy_results$pnl
  
  trades <- data.frame(Symbol = character(), Entry = integer(), Exit = integer(), Duration = integer(), PnL = numeric(), stringsAsFactors = FALSE)
  in_position <- FALSE
  entry_index <- NA
  
  for (i in 2:n) {
    if (!in_position && position[i - 1] == 0 && position[i] != 0) {
      entry_index <- i
      in_position <- TRUE
    }
    if (in_position && position[i - 1] != 0 && position[i] == 0) {
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Trade Activity", tabName = "activity", icon = icon("exchange-alt")),
    ...
    tabItems(
                ...
                tabPanel("Strategy Comparison",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Comparison Table: OU vs Buy & Hold",
                                    status = "primary",
                                    solidHeader = TRUE,
                         ...
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Comparison of Returns by Pair",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("comparison_barplot", height = "400px")
                                  )
                           )
                         )
                ),
      ...
      tabItem(tabName = "risk",
              tabBox(
                ...
                tabPanel("Drawdown Over Time",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Drawdown Over Time (Combined Portfolio)",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("drawdown_plot", height = "350px")
                                  )
                           )
                         )
                ),
                
                tabPanel("Rolling Sharpe Ratio",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    ...
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("rolling_sharpe_plot", height = "350px")
                                  )
                           )
                         )
                ),
                
                tabPanel("Drawdown by Pair",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Max Drawdown per Currency Pair",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("drawdown_pairs_plot", height = "350px")
                                  )
                           )
                         )
                ),
                
                tabPanel("Drawdown vs Volatility",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Drawdown vs Volatility",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("dd_vs_vol_plot", height = "350px")
                                  )
                           )
                         )
                )
      ...
      tabItem(tabName = "activity",
              tabBox(
                ...
                tabPanel("Number of Trades",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Number of Trades per Currency Pair",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("num_trades_plot", height = "500px")
                                  )
                           )
                         )
                ),
                
                tabPanel("Trade PnL Histogram",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    ...
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("hist_pnl", height = "350px")
                                  )
                           )
                         )
                ),
                
                tabPanel("Holding Time",
                         fluidRow(
                           column(width = 12,
                                  box(
                                    width = 12,
                                    title = "Average Holding Time per Currency Pair",
                                    status = "primary",
                                    solidHeader = TRUE,
                                    plotOutput("holding_time", height = "350px")
                                  )
                           )
                         )
                )
...
server <- function(input, output, session) {
  ...
  output$comparison_barplot <- renderPlot({
    ...
    df <- comparison %>%
      select(Symbol, OU_Strategy_Return, Buy_Hold_Return) %>%
      tidyr::pivot_longer(cols = -Symbol, names_to = "Strategy", values_to = "Return")
    
    df$Strategy <- factor(df$Strategy, levels = c("OU_Strategy_Return", "Buy_Hold_Return"),
                          labels = c("OU Strategy", "Buy & Hold"))
    
    ggplot(df, aes(x = reorder(Symbol, Return), y = Return, fill = Strategy)) +
      geom_bar(stat = "identity", position = "dodge") +
      coord_flip() +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
      labs(
        title = "Comparison of Returns: OU Strategy vs Buy & Hold",
        x = "Currency Pair",
        y = "Total Return",
        fill = "Strategy"
      ) +
  ...
  output$drawdown_plot <- renderPlot({
    ...
    # Portfolio returns = moyenne des PnL
    n_obs <- length(pnls[[1]])
    portfolio_returns <- sapply(1:n_obs, function(i) mean(sapply(pnls, function(pnl) pnl[i]), na.rm = TRUE))
    ...
    ggplot(df, aes(x = Day, y = Drawdown)) +
      geom_area(fill = "#FF4136", alpha = 0.4) +
      geom_line(color = "#FF4136", size = 1) +
      labs(
        title = "Drawdown Over Time (Combined Portfolio)",
        x = "Day",
        y = "Drawdown"
  ...
  output$rolling_sharpe_plot <- renderPlot({
    req(strategy_results())
    ...
    ggplot(df, aes(x = Day, y = RollingSharpe)) +
      geom_line(color = "#FFA500", size = 1) +
  ...
  output$drawdown_pairs_plot <- renderPlot({
    req(strategy_results())
    perf_df <- strategy_results()$performance_summary
    
    ggplot(perf_df, aes(x = reorder(Symbol, -Max_Drawdown), y = Max_Drawdown)) +
      geom_bar(stat = "identity", fill = "#FF4136") +
      labs(
        title = "Maximum Drawdown per Currency Pair",
        x = "Currency Pair",
        y = "Drawdown"
      ) +
      scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  ...
  output$dd_vs_vol_plot <- renderPlot({
    req(strategy_results())
    results <- strategy_results()$results
    symbols <- names(results)
    forex_data <- data_reactive()
    
    df <- data.frame(Symbol = character(), Volatility = numeric(), MaxDrawdown = numeric(), stringsAsFactors = FALSE)
    
    for (sym in symbols) {
      prices <- forex_data[, which(symbols == sym)]
      pnl <- results[[sym]]$strategy_results$pnl
      cum_pnl <- cumsum(ifelse(is.na(pnl), 0, pnl))
      volatility <- sd(pnl, na.rm = TRUE) * sqrt(252)
      peak <- cummax(cum_pnl)
      drawdown <- peak - cum_pnl
      max_dd <- max(drawdown, na.rm = TRUE)
      
      df <- rbind(df, data.frame(Symbol = sym, Volatility = volatility, MaxDrawdown = max_dd))
    }
    
    ggplot(df, aes(x = Volatility, y = MaxDrawdown, label = Symbol)) +
      geom_point(color = "#FF851B", size = 3) +
      geom_text(vjust = -0.8, size = 3) +
      labs(
        title = "Drawdown vs Volatility per Pair",
  ...
  output$num_trades_plot <- renderPlot({
    req(strategy_results())
    
    perf_df <- strategy_results()$performance_summary
    
    ggplot(perf_df, aes(x = reorder(Symbol, -Num_Trades), y = Num_Trades)) +
      geom_bar(stat = "identity", fill = "#0074D9") +
      labs(
        title = "Number of Trades Executed per Currency Pair",
        x = "Currency Pair",
        y = "Number of Trades"
      ) +
      theme_minimal() +
  ...
  all_trades <- reactive({
    ...
    all_trades <- do.call(rbind, lapply(symbols, function(sym) {
      prices <- data_reactive()[, which(symbols == sym)]
      extract_trades(prices, results[[sym]]$strategy_results, sym)
    }))
  ...
  output$hist_pnl <- renderPlot({
    req(all_trades())
    df <- all_trades()
    
    ggplot(df, aes(x = PnL)) +
      geom_histogram(binwidth = 0.01, fill = "#0074D9", color = "white") +
      labs(
  ...
  # Temps moyen en position par paire
  output$holding_time <- renderPlot({
    req(all_trades())
    df <- all_trades()
    ...
    ggplot(df_summary, aes(x = reorder(Symbol, Average_Duration), y = Average_Duration)) +
      geom_bar(stat = "identity", fill = "#FF851B") +
      coord_flip() +
      labs(
        title = "Average Holding Time (in days) per Currency Pair",
        x = "Currency Pair",
        y = "Average Duration"
      ) +
      theme_minimal() +
